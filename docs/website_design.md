...
- **Staging must mirror production.** Staging exists to verify what production will do, so anything withheld from staging is something that cannot be verified before it ships. This matters most for the things you cannot see by looking at the page - structured data, metadata, crawler directives - because staging is the only place to check how integrators, search engines, and LLMs will read the site.
  ...
  - Parity is about the *hostname* too, not just the markup. Zone-level features (Cloudflare Fonts, cache rules, redirect rules) apply only to hostnames inside the `ontariomenopauseclinic.ca` zone, so a `*.pages.dev` URL cannot verify them - a font change was once reviewed on staging and turned out to be a no-op in production for exactly this reason (#128). The default staging slot is therefore served in-zone at `staging.ontariomenopauseclinic.ca`; see `docs/cloudflare-config.md`. Numbered slots stay on `pages.dev` and are fine for comparing UX concepts, but anything touching zone behaviour must be checked in-zone.
...
- Trained at Sinai under Dr. Wolfman, Dr. Christakis and Dr. Shirreff  
  - … and I decided to start my own clinic so as to do the treatment better for women
  - Medicine knows the { syndrome, symptom, etc } really well, but medicine doesn’t always see the whole woman - it’s not just your hormones, it’s your life
...
| Business | Site URL |
|----------|----------|
| Modern Menopause | https://www.modernmenopause.ca/ |
| Bravella | https://bravellahealth.com/ |
| Coral | https://coral.ca/ |
| Blair Health | https://blairhealth.ca/ |
| Medcan | https://medcan.com/ |
| Midi Health | https://www.joinmidi.com/
| Bria | https://betterbria.com/ |
| The Virtual Menopause Clinic | https://www.virtualmenopauseclinic.com/ |
| Dr. Arnold Signature Wellness | http://signaturewellness.ca/ |
